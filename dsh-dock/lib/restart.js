/**
 * dsh-dock — process control half (merged from dsh-restart).
 *
 ...
 * plus the `/dsh-dock/restart` and `/dsh-dock/stop` web endpoints used by the
 * dock buttons.
 *
...
const RESTARTING_FLAG_FILENAME = 'dsh-dock-restarting.flag';
function homeDir() {
...
function writeResumeMarker(sessionIds) {
    try {
        ...
        console.error('[dsh-dock] failed to write resume marker:', error);
    }
}
...
function debugLog(message) {
    try {
        fs.appendFileSync(path.join(homeDir(), 'dsh-dock-auto.log'), `${new Date().toISOString()} ${message}\n`, 'utf8');
    }
    catch { /* best-effort */ }
}
    ...
    const interval = setInterval(() => {
        ...
        for (const sessionId of [...pending]) {
            ...
            try {
                ...
                console.error('[dsh-dock] auto-continue failed:', error);
...
const restartFlag = path.join(home, 'dsh-dock-restarting.flag')
...
function relaunch() {
  ...
  try {
    ...
    // node-native relaunch with windowsHide (detached survives parent exit;
    // windowsHide prevents console-child popups). A powershell wrapper was
    // tried but failed to relaunch reliably.
...
function ensureWatchdog(dynamic) {
    ...
    try {
        ...
        console.error('[dsh-dock] failed to spawn watchdog:', error);
    }
}
...
function writeProcessIndex() {
    try {
        ...
        console.error('[dsh-dock] failed to write process index:', error);
    }
}
/**
...
function restart(delayMs) {
    ...
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const logOut = path.join(os.tmpdir(), `dsh-dock-${stamp}.out.log`);
    const logErr = path.join(os.tmpdir(), `dsh-dock-${stamp}.err.log`);
    ...
    // `detached:true` + `windowsHide:true`: detached survives the parent exit;
    // windowsHide keeps the new host's own console hidden and prevents the
    // console-child popups. (A powershell -WindowStyle Hidden wrapper was tried
    // but failed to relaunch reliably; node-native spawn is the proven path.)
    const spawnLine = `const child = spawn(process.execPath, argv, { cwd: cwd, detached: true, windowsHide: true, stdio: ["ignore", out, err], env: process.env })`;
    const helperCode = [
        "const { spawn } = require('node:child_process')",
        "const fs = require('node:fs')",
...
$logOut = Join-Path $env:TEMP ("dsh-dock-" + $stamp + ".out.log")
$logErr = Join-Path $env:TEMP ("dsh-dock-" + $stamp + ".err.log")
...
$helperPath = Join-Path $env:TEMP 'dsh-dock-helper.ps1'
...
export function registerRestart(ctx) {
    ...
    try {
        installSettingsSection(ctx, settingsNamespace('dsh-dock'), RestartConfigSchema, DEFAULT_CONFIG, {
    ...
    if (webServer !== undefined) {
        ...
        }), 'dsh-dock: same-origin restart endpoint');
        ctx.effect(() => webServer.register({
            kind: 'exact',
            path: '/dsh-dock/stop',
            handler: (req, res) => {
                ...
                // Graceful shutdown: give the response time to flush, then exit.
                setTimeout(() => process.kill(process.pid, 'SIGTERM'), 400);
            },
        }), 'dsh-dock: same-origin stop endpoint');
    }
    try {
